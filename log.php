...
echo "student exists $row[stu_roll_no]\n";
...
if($in_out!=0){               //student is already inside building
	...
	*/
	...
	if($gate_type=="regional_gate"){
		...
		//error 1 if student is transacting on gate which is not in current region
			...
			if($old_region!=$old_region_1 && $old_region!=$old_region_2){
				mysql_query("insert into error values ($new[0],$old_transaction_id)");
	...
	//decrease number of students in old location
		...
		echo "$location[region_id1] $location[region_id2] $location[gate_name]\n";
		$current=mysql_fetch_array(mysql_query("select region_id from current_status where stu_roll_no='$roll_no'"));
		if($current['region_id']==$location['region_id1'])
			{
			if(!mysql_query("update region set r_number_of_students=r_number_of_students-1 where region_id=$location[region_id1]")) echo "number of students not decremented\n";
			//increase number of students in new location
			if(!mysql_query("update region set r_number_of_students=r_number_of_students+1 where region_id=$location[region_id2]")) echo "number of students not incremented\n";
	//set current status of student
			if(!mysql_query("update current_status set region_id=$location[region_id2],building_id=0,transaction_id=$current_status[0] where stu_roll_no='$roll_no'")) echo "status not updated\n";
		}
		else{
			if(!mysql_query("update region set r_number_of_students=r_number_of_students+1 where region_id=$location[region_id1]")) echo "number of students not decremented\n";
			//increase number of students in new location
			if(!mysql_query("update region set r_number_of_students=r_number_of_students-1 where region_id=$location[region_id2]")) echo "number of students not incremented\n";
			//set current status of student
			if(!mysql_query("update current_status set region_id=$location[region_id1],building_id=0,transaction_id=$current_status[0] where stu_roll_no='$roll_no'")) echo "status not updated\n";
		}
